...
export async function withRetry<T>(fn: () => Promise<T>, retries = 3, delay = 1500): Promise<T> {
  try {
    ...
    console.warn(`[Gemini Service] Errore temporaneo, riprovo tra ${delay}ms... (${retries - 1} tentativi rimasti). Errore:`, errorMessage);
    await new Promise(resolve => setTimeout(resolve, delay));
    return withRetry(fn, retries - 1, delay * 2.0); // Tighter exponential backoff to fit within connection gateway limits
  }
}
...
export async function identifyPlant(
  ...
  return withRetry(async () => {
    try {
      ...
      try {
        console.log(`[Gemini Service] Chiamata a generateContent con modello: ${activeModel}`);
        ...
        if (isTransient) {
          ...
          console.warn(`[Gemini Service] Impossibile usare ${activeModel} a causa del carico. Fallback automatico ed immediato a: ${activeModel}...`);
          const response = await ai.models.generateContent({
            model: activeModel,
            contents,
            config: {
          ...
          const text = response.text;
          if (!text) throw new Error("Nessuna risposta ricevuta dal modello di fallback.");
          return JSON.parse(text.trim());
        }
        throw error;
      }
...
export async function searchPlant(
  ...
  return withRetry(async () => {
    try {
      ...
      let response;
      try {
        console.log(`[Gemini Service] Chiamata a generateContent per Ricerca con modello: ${activeModel}`);
        ...
        if (isTransient) {
          activeModel = "gemini-3.1-flash-lite";
          console.warn(`[Gemini Service] Ripiego di emergenza per Ricerca su modello: ${activeModel}. Dettagli errore originale: ${error?.message || error}`);
          response = await ai.models.generateContent({
            model: activeModel,
            contents: [{ role: "user", parts: [{ text: prompt }] }],
          });
